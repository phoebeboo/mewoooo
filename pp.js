            ...
            if (xProfile && xProfile.publicIdentity) {
              ...
              if (isCharPublicFigure) {
                characterProfiles.push({
                  ...
                  xProfile: xProfile, // 保存完整资料以便后续读取推文
                });
              }
            }
          }
...
- X姓名：${char.name}
- X句柄：${char.handle}
- 公众身份：${char.publicIdentity}
${char.xProfile.xBio ? `- X简介：${char.xProfile.xBio}` : ''}
`;

              // 读取角色的最近5条推文（从角色的账户主页数据中）
              try {
                const cleanCharHandle = char.handle.replace('@', '');
                ...
                if (recentCharTweets.length > 0) {
                  systemPrompt += `
${char.name} 的近期推文（${recentCharTweets.length}条）：
`;
...
${tweet.media && tweet.media.length > 0 ? `   - 媒体：${tweet.media[0].description.substring(0, 50) + '...'}\n` : ''}`;
                ...
                console.warn(`热搜生成器：读取角色 ${char.name} 的推文失败:`, error);
              }
...
`;
  ...
  window.showSearchTweetDetail = async function (tweetId) {
    console.log('📖 [搜索结果] 显示推文详情:', tweetId);
...
${tweet.media && tweet.media.length > 0 ? `   - 媒体：${tweet.media[0].description.substring(0, 50) + '...'}\n` : ''}`;
            ...
            console.warn(`搜索生成器：读取角色 ${xProfile.xName} 的推文失败:`, error);
          }
...
`;
        ...
        tokenCount = TokenUtils.logTokenUsage('搜索生成器', '允许角色信息与推文', charSection, tokenCount);
      }
...
`;
      ...
      tokenCount = TokenUtils.logTokenUsage('搜索生成器', '用户资料约束与推文', userConstraints, tokenCount);
...
4. content直接写内容，不用引号包裹`;
  ...
  window.openAccountProfile = async function (accountName, accountHandle, accountAvatar, sourceContext = {}) {
    try {
      ...
      console.log('📍 来源信息:', sourceContext);
...
现在，请生成JSON格式的问答内容：`;
  ...
  async function generateAIResponseForTweet(tweetData, isReroll = false, isProgressMode = false) {
    try {
      ...
      if (isAccountTweet) {
        // 账户推文：构建账户资料信息
        console.log('🔍 [AI生成] 检测到账户推文，加载账户资料');
        ...
        if (accountData) {
          ...
          console.log('✅ [AI生成] 已加载账户资料:', targetProfileInfo.name);
          ...
          const imagesHtml = currentQuoteData.image.images
            .map((img, index) => {
              ...
              <div style="border-radius: 6px; overflow: hidden;">
                <img src="${
                  img.content
                }" style="width: 100%; height: 80px; object-fit: cover; display: block;" alt="引用图片${index + 1}">
              </div>
            `;
            })
            .join('');
